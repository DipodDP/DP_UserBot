...
async def resend(app: Client, message: Message, channel_id: int):
    ...
    # printing some info
    LOGGER.debug(getattr(message, "text", 'no text').split('\n')[0], message.caption, getattr(message, "chat.id", None),
                 getattr(message, "title", None), getattr(getattr(message, "from_user", None), "first_name", None),
